<header class="dynamic-header shadow-md sticky top-0 z-40 transition-colors duration-400">
    <div class="max-w-7xl mx-auto px-4 h-16 flex justify-between items-center">
        <a href="<?= BASE_URL ?>/" class="flex items-center space-x-2 title-logo">
        ...
        <div class="flex items-center space-x-3">
            ...
            <?php if ($this->isLogged()): ?>
                ...
                <?php if ($role === ROLE_ADMIN): ?>
                    <a href="<?= BASE_URL ?>/panel/admin" class="px-3 py-2 rounded-lg hover:bg-black/5 transition font-medium">Panel admin</a>
                <?php elseif ($role === ROLE_VENDEDOR): ?>
                    <a href="<?= BASE_URL ?>/panel/vendedor" class="px-3 py-2 rounded-lg hover:bg-black/5 transition font-medium">Panel vendedor</a>
                <?php elseif ($role === ROLE_SOPORTE): ?>
                    <a href="<?= BASE_URL ?>/panel/soporte" class="px-3 py-2 rounded-lg hover:bg-black/5 transition font-medium">Panel soporte</a>
                <?php endif; ?>

                <a href="<?= BASE_URL ?>/mis-compras" class="px-3 py-2 rounded-lg hover:bg-black/5 transition font-medium">Mis compras</a>

                <a href="<?= BASE_URL ?>/logout" class="cl-sesion flex items-center space-x-1 px-4 py-2 rounded-lg hover:bg-black/5 transition font-medium">
                ...
                <?php endif; ?>
            <?php else: ?>
                <a href="<?= BASE_URL ?>/login" class="flex items-center space-x-1 px-4 py-2 rounded-lg hover:bg-black/5 transition font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
