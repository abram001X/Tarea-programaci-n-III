...
<script type="module">
    ...
    // 1. Corregimos la asignación de sesión de PHP a un booleano real de JS
    const isLogin = <?php echo isset($_SESSION["name"]) && isset($_SESSION["email"]) ? "true" : "false"; ?>;
    console.log(isLogin)
    // Array global temporal para guardar los productos añadidos al carrito
    let carrito = []; 
    // Guardamos la lista completa de productos de la API para buscarlos luego por ID
    let listaProductos = []; 

    async function render() {
        // Guardamos los productos en la variable global para poder buscarlos al añadir al carrito
        listaProductos = await getProducts();
        const container = document.getElementById('product-grid');
        
        container.innerHTML = listaProductos.map(p => `  
            <div class="dynamic-card rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group">
                <div class="relative overflow-hidden aspect-square">
                    ...
                    <!-- El botón ahora llama a una función global de window -->
                    <button onclick="if(${isLogin}) { window.addToCart(${p.id}) } else { location.href='indexlogin.php' }" class="absolute bottom-4 right-4 bg-indigo-600 text-white p-3 rounded-xl shadow-lg transform translate-y-12 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
    ...
    // 2. NUEVA FUNCIÓN: Agregar al carrito
    function addToCart(id) {
        // Buscamos el producto en la lista que descargamos de la API
        const productoEncontrado = listaProductos.find(p => p.id === id);
        
        if (productoEncontrado) {
            // Lo añadimos al array del carrito
            carrito.push(productoEncontrado);
            
            // Actualizamos la interfaz visual del carrito
            updateCartUI();
            
            // Opcional: Abre automáticamente el carrito para que el usuario vea que se agregó
            toggleCart();
        }
    }
    ...
    // 3. CORRECCIÓN de tu función updateCartUI
    function updateCartUI() {
        // Eliminamos la línea "const carrito = []" que borraba tu carrito en cada renderizado
        ...
        const total = carrito.reduce((s, x) => s + parseFloat(x.precio), 0);
        // Corregido el operador de asignación '=' (tenías '==')
        if (cartTotal) cartTotal.innerText = `$${total.toFixed(2)}`;
        
        if (carrito.length === 0) {
            items.innerHTML = '<p class="text-center opacity-50 py-10">Vacío</p>';
        } else {
            // Nota: Cambié x.img por x.imagen para que coincida con la propiedad de tu API
            items.innerHTML = carrito.map((x, i) => `
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <button onclick="window.removeFromCart(${i})" class="text-red-400 hover:text-red-600 transition">
    ...
    // FUNCIÓN EXTRA: Para que el botón de eliminar del carrito funcione
    function removeFromCart(index) {
        carrito.splice(index, 1);
        updateCartUI();
    }

    // Exponemos las funciones al objeto global window para que los atributos 'onclick' del HTML puedan verlas
